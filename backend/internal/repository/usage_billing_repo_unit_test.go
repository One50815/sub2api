...
func TestAllocateOmnioProFreeQuota_SharesUsageAcrossGroupsAndSplitsAtBoundary(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	tx, err := db.BeginTx(ctx, nil)
	require.NoError(t, err)
	mock.ExpectQuery(`(?s)SELECT e\.id, e\.daily_free_usd, e\.monthly_free_usd.*FROM effective_level e`).
	...
	mock.ExpectExec(`(?s)INSERT INTO omnio_pro_level_shared_quota_usage`).
		WithArgs(int64(7), int64(4), currentDay, currentMonth).
	...
	mock.ExpectExec(`(?s)UPDATE omnio_pro_level_shared_quota_usage`).
		WithArgs(int64(7), int64(4), currentDay, currentMonth, 10.0, 100.0).
