...
> 状态：生效中  
> 最后核对日期：2026-08-02
> 基线：`main`，`f49c45e05`（版本 `0.1.169`；官方提交 `26d894ef4`）
...
| 2026-07-24 | `sub2api:custom-20260724-omnio-pro-shared-quota` / `9591014c` | 修正 Omnio Pro 免费额度语义并增加权限撤销管理：每日和每月上限移到会员等级；同一用户当前有效等级的 A/B/C 或更多允许访问分组共同扣减一份额度，计数主键固定为 `(user_id, level_id)`；分组倍率、访问、Pro 专属与 RPM 仍按等级和分组独立配置。后台新增权限记录列表、用户/状态筛选和单笔软撤销，撤销保留审计且不影响其他发放记录 | 迁移 192 将旧分组额度的最大值提升到等级，并把当日/当月旧分组用量求和写入共享表，旧表保留回退兼容；生产共享表回填 12 条，主键为 `{user_id,level_id}`，旧用量缺失/少计数为 0。备份位于 `/opt/sub2api/backups/20260724T114909Z-pre-omnio-pro-shared-quota/`，包含已验证恢复清单的数据库 dump、源码包、旧镜像、容器 inspect 与校验和；仅替换 `sub2api`，PostgreSQL `7f93ea7258cb` 与 Redis `303ee63c404a` 未重建，旧镜像 `sub2api:custom-20260724-group-member-rates` 保留可回退 |

| 2026-07-25 | 生产账号配置 | 通过后台批量创建接口新增 5 个 Anthropic API Key 账号：`cc max-8c3dwq`（ID 40）、`cc max-tr2w3g`（ID 41）、`cc max-qfwxv3`（ID 42）、`cc max-m40ubm`（ID 43）、`cc max`（ID 44）；统一使用 `https://ai.zyyun.xyz`，并发 10、优先级 1、账号倍率 1，密钥不写入本文档 | 生产当前没有 Anthropic 分组，因此 5 个账号保持未分组、`active`、可调度；5 个密钥查询 `/v1/models` 均返回 200，并开放 `claude-sonnet-4-6` 等模型。使用 `claude-sonnet-4-6` 实际转发测试时 ID 42 成功，ID 40/41/43/44 连续两次由上游返回 502 `Upstream service temporarily unavailable`；默认测试模型 `claude-sonnet-4-5-20250929` 不在该上游开放列表，不应据此判定密钥失效 |
...
| 2026-08-02 | `sub2api:custom-20260802-0.1.169-ai-image` / `f49c45e05` | 将官方 `0.1.169` 与 Omnio 原生 AI 绘图一并部署生产；镜像版本信息为 `0.1.169 / f49c45e05`，`/image-generation`、Omnio Pro、模型广场和原批量生图页面均返回 200，图片模型、异步生成和任务查询接口在无密钥时正确返回 401 | 发布前备份位于 `/opt/sub2api/backups/20260801T194840Z-pre-0.1.169-ai-image/`，包含已校验数据库 dump、源码配置、旧镜像、容器 inspect 与 SHA256 清单。首次在 4 GB 生产机执行前端 Docker 构建造成内存压力并通过控制台强制重启恢复，随后改用本地已验证前端产物、临时 4 GB swap 和 Go `-p=2` 完成低内存构建；临时 swap 已删除。最终只重建 `sub2api`，PostgreSQL `7f93ea7258cb` 与 Redis `303ee63c404a` 容器 ID 未变且均 healthy；官方迁移 187–191 与 Fork 同号迁移并存，内外 `/health` 均为 `{"status":"ok"}`，新应用重启次数为 0，未执行会消耗余额的真实图片生成 |
| 2026-08-02 | `sub2api:custom-20260802-0.1.169-ai-image-hotfix1` / `9c48e0250` | 修复原生 AI 绘图的两个上线问题：模型控件改为明确可见的下拉选择器，并按 `gpt-image-2`、`gpt-image-1.5`、`gpt-image-1` 的顺序优先展示和默认选择；ChatGPT Codex OAuth 图片请求不再强制提交对象形式的 `image_generation` tool choice，改为与现有 Codex 图片桥接一致的 `tool_choice: "auto"` 与显式图片工具指令，解决上游 `Tool choice 'image_generation' not found in 'tools' parameter.` | 前端定向测试 7 项、lint、typecheck、生产构建和后端 `internal/service` 全包测试通过；使用本机交叉编译的静态 Linux 二进制组装镜像，避免再次在 4 GB 生产机执行高内存前端构建。发布前配置、容器与旧镜像备份位于 `/opt/sub2api/backups/20260802T090400Z-pre-image-hotfix1/`，旧镜像另有 `sub2api:rollback-20260802-pre-image-hotfix1` 标签。最终只重建 `sub2api`，PostgreSQL/Redis 原容器 ID 未变；应用 healthy、重启次数为 0、`no-new-privileges:true` 生效，内外 `/health` 均正常，线上静态资源确认 `gpt-image-2` 优先且 `/v1/models` 包含该模型。未执行会产生费用的真实图片生成，OAuth 修复由请求构造回归测试覆盖，发布后应由用户正常生成流程完成首笔业务验收 |
| 2026-08-10 | `sub2api:custom-20260810-frontend-redesign` / `34bad06b9d-ui-20260809` | 将公开首页和完整认证入口更新为新视觉：公开首页采用暖中性色、绿色单一强调色与更克制的信息层级，核心表达统一为明文阅后即焚、模型选择透明和费用透明；登录、注册及共享认证辅助页采用新的左右分栏和表单系统，同时保留 Passkey、OAuth、Turnstile、协议、邀请码、优惠码、2FA 与邮箱验证契约；后台保持原样 | 本地 lint、类型检查、鉴权与 i18n 定向测试此前已通过，本次发布门禁重新完成生产构建与 Go `1.26.5` Linux 静态交叉编译。发布包 SHA-256 与服务器一致，发布前备份位于 `/opt/sub2api/backups/20260809T155923Z-pre-frontend-redesign/`，数据库 dump 的 `pg_restore --list` 清单为 1261 行，旧镜像保留为 `sub2api:rollback-20260809-pre-frontend-redesign`。最终仅重建 `sub2api`，PostgreSQL/Redis 容器 ID 未变；应用 healthy、重启次数为 0、`no-new-privileges:true` 生效，内外 `/health` 正常，`/`、`/login`、`/register` 与新首页/认证 JS、CSS 资源均返回 200，启动窗口实际 `ERROR/FATAL/PANIC` 为 0 |
| 2026-08-11 | `native-ai-image-playground` | 将 `/image-generation` 从左右卡片式表单重构为 New API 风格的原生生图游乐场：采用全高结果画布、底部统一提示词输入器、集成 API Key 与模型选择、可展开尺寸和数量参数、连续生成轮次、停止生成、提示词复用及单图/本轮下载；侧栏名称同步为“生图游乐场” | 保留既有模型发现、异步任务轮询、组合分组同步回退、余额刷新与网关计费契约，不新增后端接口或 iframe。定向 ESLint、类型检查、图片 API/页面测试、国际化测试和生产构建通过；使用本地 mock 完成空状态、参数面板、多图结果及浅/深主题浏览器验收，未调用真实付费生图，未发布生产 |
| 2026-08-13 | 官方 `0.1.176` / `e803e3851` | 在 `sync/upstream-20260813` 合并官方 `v0.1.176`，吸收 Grok 更新、渠道监控 v2、分组利润控制、腾讯云/阿里云验证码、计费与支付安全修复、模型级定价及迁移 192–221；重新生成 Ent 与 Wire，并修复代理环境下阿里云验证码 SDK 测试 | 保留 Omnio 新首页与认证视觉、工单、Omnio Pro、会员共享额度、公开订阅退役边界和原生生图游乐场；首页同时恢复自定义内容优先与紧凑首页兼容，登录保留稳定 Passkey 选择器，分组页同时校验会员权益与利润控制。前端 lint、typecheck、完整 Vitest 和生产构建通过，后端 server/service/handler/repository/server/payment/securityaudit 关键包与阿里云验证码测试通过 |
| 2026-08-14 | `sub2api:custom-20260813-0.1.176` / `9a6efd7b9390` | 将官方 `0.1.176` 与 Omnio 定制能力部署生产；使用本地 Go `1.26.5` 交叉编译含前端资源和迁移的 Linux amd64 静态二进制，SHA-256 为 `56b078fba27cb28cd44e56f7cda04558ac79e03790b72ad42c62bd41c07c2468`，服务器以旧运行时镜像组装候选并完成版本与二进制校验 | 完整备份位于 `/opt/sub2api/backups/20260813T115133Z-pre-0.1.176/`，包含经 `pg_restore --list` 验证的数据库 dump、源码配置、旧镜像、容器 inspect 和 SHA256 清单，旧镜像另保留为 `sub2api:rollback-20260813-pre-0.1.176`。最终仅重建 `sub2api`，PostgreSQL `7f93ea7258cb` 与 Redis `303ee63c404a` 容器 ID 未变且 healthy；应用版本为 `0.1.176 / 9a6efd7b9390`、重启次数 0、`no-new-privileges:true` 生效，相关 23 个完整迁移文件已落库；内外 `/health` 及 `/`、`/login`、`/register`、`/image-generation`、`/omnio-pro`、`/admin/omnio-pro`、`/model-plaza` 均返回 200，近 10 分钟无 panic、fatal、迁移或校验错误 |
...
- 当前公开首页入口：`frontend/src/views/HomeView.vue`；首页样式与轻量交互：`frontend/src/styles/home-v2.css`；品牌标记复用 `frontend/src/components/home-v2/OmnioMark.vue`。当前方案已移除上一版 Pixel Mascot、像素场景和装饰性素材，以暖中性色、绿色强调色、真实模型节点和清晰信息层级表达隐私、模型透明与费用透明；
- 原生 AI 绘图入口：`frontend/src/views/user/ImageGenerationView.vue`；接口契约：`frontend/src/api/imageGeneration.ts`；用户路由：`/image-generation`；继续复用现有 API Key、网关图片接口与余额计费体系，不引入 iframe；
...
- 认证外壳：`frontend/src/components/layout/AuthLayout.vue`；登录与注册页：`frontend/src/views/auth/LoginView.vue`、`RegisterView.vue`；统一认证样式：`frontend/src/styles/auth-v2.css`。覆盖登录、注册、忘记密码、重置密码、邮箱验证、2FA 和 OAuth 过程页，并保留浅色/深色、中文/英文与移动端兼容；
...
- 2026-08-02 `0.1.169 + 原生 AI 绘图` 生产发布门禁：发布包 SHA256 与服务器接收文件一致；数据库自定义格式备份可由 `pg_restore --list` 读取，源码配置和旧镜像归档均有校验和；低内存镜像预检确认可执行文件、计价资源、嵌入式 `/image-generation` 内容与版本元数据存在。新容器 healthy、重启次数为 0、`no-new-privileges:true` 生效；PostgreSQL/Redis 原容器 ID 未变；`schema_migrations` 已登记官方 `187_add_usage_log_session_id.sql` 至 `191_passkey_credentials.sql`，并保留 Fork 同号迁移；首页、AI 绘图、Omnio Pro、管理员 Omnio Pro、模型广场和批量生图路由均返回 200，三条图片 API 鉴权冒烟返回 401；启动日志无 error/fatal/panic，内外 `/health` 均返回 `{"status":"ok"}`。生产双 Compose 叠加时新版基础文件和 local 文件会重复合并 `security_opt`，因此生产 `docker-compose.local.yml` 删除重复项，并由基础文件及 custom override 保留唯一的 `no-new-privileges:true`；未来同步 Compose 时必须继续核对该生产兼容调整。
- 2026-08-02 AI 绘图上线热修门禁：`frontend/src/api/__tests__/imageGeneration.spec.ts` 共 7 项通过，前端 lint/typecheck/生产构建通过，`go test ./internal/service -count=1` 通过，`git diff --check` 通过；生产包 SHA256 与服务器接收值一致，镜像预检确认版本为 `0.1.169 / 9c48e0250`，`psql`/`pg_dump` 均为 18.4。切换后应用 healthy、重启次数为 0，PostgreSQL/Redis 容器 ID 未变；线上 `ImageGenerationView-Do_Msz-2.js` 同时包含明确模型选择器和 `gpt-image-2` 优先列表，已授权 `/v1/models` 返回 `gpt-image-1`、`gpt-image-1.5`、`gpt-image-2`，内外 `/health` 均返回 `{"status":"ok"}`，启动窗口无 error/fatal/panic。为避免消耗用户余额与上游额度，未执行真实生图；OAuth 请求兼容性由后端回归测试验证。
- 2026-08-10 首页与认证页重做生产发布门禁：本地 `pnpm run build` 成功转换 1026 个模块，Go `1.26.5` 使用 `CGO_ENABLED=0` 交叉编译出 Linux amd64 静态二进制，二进制 SHA-256 为 `f971d51034cb57b00b9ae5802407d602df0977ed6c84425fdbe482f9c2e0db6e`，发布包 SHA-256 为 `d25a5e6700fba00d15db2f1139317863bf0c255cc01322f6373f278670baa5d5`。服务器镜像预检确认版本 `0.1.169 / 34bad06b9d-ui-20260809`、新 `HomeView`/`LoginView`/`RegisterView` 资源与 PostgreSQL 18.4 客户端工具存在；切换后应用 healthy、重启次数 0，PostgreSQL/Redis 原容器 ID 未变，内外健康检查及三个公开路由、新 JS/CSS 资源均返回 200，启动窗口实际严重日志为 0。
- 2026-08-11 生图游乐场门禁：受影响文件定向 ESLint 与 `pnpm run typecheck` 通过；图片 API 与页面共 2 个测试文件、10 项通过；中英文国际化编译与键冲突共 2 个测试文件、8 项通过；`pnpm run build` 成功转换 1026 个模块，仅保留项目既有分包、chunk 大小与 Browserslist 警告。本地 mock 浏览器验收覆盖空状态、参数展开、异步生成停止态、四图结果和浅/深主题；未调用真实付费生图，未发布生产。
