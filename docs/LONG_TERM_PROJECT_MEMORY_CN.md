...
- Omnio v2 首页入口：`frontend/src/views/HomeView.vue`；首页模块：`frontend/src/components/home-v2/`；独立样式与动效：`frontend/src/styles/home-v2.css`；唯一品牌角色素材：`frontend/public/assets/brand/omnio-pixel-mascot-connector-v2.png`。首页不引入像素城市、重复角色、3D 卡通 Mascot 或大面积背景插画；
